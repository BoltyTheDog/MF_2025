...
def plot_scalar_field(xi, yi, zi, mask_xi, mask_yi, mask_zi, title, cmap, label, filename, 
    ...
    # Add improved text annotations to explain the simulation context
    description = f"Flow visualization: {title.lower()}"
...
def plot_velocity_field(x, y, u, v, mask_xi, mask_yi, mask_zi, title, filename, downsample=15):
    ...
    ax.text(0.02, 0.02, f"Vector field visualization: {title.lower()}", 
...
def plot_streamlines(x, y, u, v, mask_xi, mask_yi, mask_zi, title, filename, density=1.5):
    ...
    # Add more detailed simulation information
    description = (f"Streamline visualization: fluid flows along the streamlines, "
                  f"with color indicating velocity magnitude")
    ax.text(0.02, 0.02, description, transform=ax.transAxes, fontsize=9,
            bbox=dict(facecolor='white', alpha=0.7, boxstyle='round,pad=0.3'))
...
def create_lagrangian_animation(x, y, u, v, mask_xi, mask_yi, mask_zi,
    ...
    ax.set_title("Fluid Flow Particle Visualization", fontsize=16, pad=10, fontweight='bold')
    ax.set_xlabel("x [m]", fontsize=12)
    ax.set_ylabel("y [m]", fontsize=12)
    ax.set_aspect('equal')
    ...
    # Add title with shadow effect for better visibility
    title = ax.text(0.5, 1.02, "Lagrangian Particle Flow Visualization", 
...
def create_pressure_coefficient_plot(x, y, p, mask_xi, mask_yi, mask_zi, v_inf, filename):
    ...
    ax.text(0.02, 0.02, 
           ...
           "Negative values indicate suction regions (higher velocity)",
           transform=ax.transAxes, fontsize=9,
           bbox=dict(facecolor='white', alpha=0.7, boxstyle='round,pad=0.3'))
